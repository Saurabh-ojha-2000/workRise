...
function AuthWrapper({ type }) {
  ...
  const handleClick = async () => {
    try {
      ...
      // Check if err.response and err.response.data exist before accessing them
      const errorMessage = err.response?.data?.message || "Invalid Credientials.";
      setError(errorMessage); // Set error message received from the server or a default message
    }
  };
  

  // useEffect to handle modal and blur div
  useEffect(() => {
    const html = document.querySelector("html");
    const authModal = document.querySelector("#auth-modal");
    const blurDiv = document.querySelector("#blur-div");
